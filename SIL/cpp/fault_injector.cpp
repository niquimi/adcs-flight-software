...
static void injectModeReplica(FlightSoftware& fsw, float timestamp_s) {
...
void FaultInjector::apply(SensorPacket& sensors, FlightSoftware& fsw) {
    if (!mode_done_ && mode_at_s_ >= 0.f && sensors.timestamp_s >= mode_at_s_) {
        injectModeReplica(fsw, sensors.timestamp_s);
        mode_done_ = true;
    }
...
void FaultInjector::fire(std::uint8_t arg0, SensorPacket& sensors, FlightSoftware& fsw) {
    if (arg0 == 1) {
        injectModeReplica(fsw, sensors.timestamp_s);
        return;
    }
    ...
    std::cout << "TC INJECT_FAULT ignored arg0=" << static_cast<int>(arg0) << "\n";
}
